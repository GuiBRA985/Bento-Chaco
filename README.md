# Tchaco Pantaneiro

Site pronto para GitHub Pages.

1. Envie todos os arquivos desta pasta para a raiz do novo repositório.
2. Coloque as fotografias dentro da pasta `assets`.
3. Use exatamente os nomes e tamanhos indicados em `GUIA-DE-IMAGENS.md`.
4. No GitHub, abra **Settings → Pages**, escolha **Deploy from a branch**, selecione `main` e `/ (root)`.

Enquanto uma fotografia não existir, o site mostra o nome do arquivo esperado. Depois que ela for enviada, aparece automaticamente.
